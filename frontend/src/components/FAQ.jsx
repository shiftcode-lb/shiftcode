...
const FAQ = () => {
  ...
  },
  {
    ...
    return (
        <>
            <div className='w-full mx-auto flex flex-col items-center text-center text-slate-800 px-3 sm:px-4 md:px-11 lg:px-13 xl:px-12 2xl:px-16 max-w-screen-xl'>
                <Title 
  ...
  <div key={index} className='flex flex-col items-center w-full text-center' data-aos="fade-up" data-aos-delay="200">
    <div
  className='relative w-full cursor-pointer border border-borderColor p-4 rounded text-center'
  onClick={() => setOpenIndex(openIndex === index ? null : index)}
>
  <h2 className='text-sm w-full '>{faq.question}</h2>
  <svg
    width="18"
    height="18"
    viewBox="0 0 18 18"
    ...
    className={`absolute right-4 top-1/2 -translate-y-1/2 transition-all duration-500 ease-in-out ${openIndex === index ? "rotate-180" : ""}`}
  >
    <path
    ...
    <p className={`text-sm text-coprimary px-4 text-center transition-all duration-500 ease-in-out ${openIndex === index ? "opacity-100 max-h-[300px] translate-y-0 pt-4" : "opacity-0 max-h-0 -translate-y-2"}`} >
      {faq.answer}
    </p>
  </div>
))}
