...
const FAQ = () => {
  ...
  },
  {
    ...
    return (
  <>
    <div className='flex flex-col justify-center items-center w-full mx-auto px-3 sm:px-4 md:px-11 lg:px-13 xl:px-12 2xl:px-16 max-w-screen-xl text-slate-800 text-center'>
      <Title 
      ...
      <div className=' w-full mt-6 flex flex-col gap-4 '>
        {faqsData.map((faq, index) => (
          <div key={index} className='flex flex-col items-center text-center w-full' data-aos="fade-up" data-aos-delay="200">
            <div 
  className='flex items-center justify-between w-full cursor-pointer border border-borderColor p-4 rounded'
...
>
  ...
  >
    <path d="m4.5 7.2 3.793 3.793a1 1 0 0 0 1.414 0L13.5 7.2" stroke="#1D293D" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round" />
            ...
            <p className={`text-sm text-coprimary px-4 transition-all duration-500 ease-in-out ${openIndex === index ? "opacity-100 max-h-[300px] translate-y-0 pt-4" : "opacity-0 max-h-0 -translate-y-2"}`} >
              {faq.answer}
            </p>
          </div>
        ))}
