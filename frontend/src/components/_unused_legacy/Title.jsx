...
const Title = ({title, subtitle, align, theme = "dark"}) => {
  useEffect(() => {
      AOS.init({
        duration: 1000,
  ...
  const headingColor = theme === "light" ? "text-background" : "text-text";
  const subColor = theme === "light" ? "text-lightMuted" : "text-muted";
  return (
    <div className={`flex flex-col justify-center items-center text-center
    ${align === "left" && "md:items-center md:text-left"}`}>
      <span className="font-mono-code text-primary text-xs md:text-sm tracking-[3px] uppercase mb-2" data-aos="fade-up" data-aos-delay="100">// {title}</span>
      <h1 className={`font-semibold text-3xl md:text-[40px] ${headingColor} mb-2`} data-aos="fade-up" data-aos-delay="200">{title}</h1>
      <p className={`text:sm md:text-base ${subColor} mt-2 max-w-156 text-[18px]`} data-aos="fade-up" data-aos-delay="200">{subtitle}</p>
    </div>
  )
}
