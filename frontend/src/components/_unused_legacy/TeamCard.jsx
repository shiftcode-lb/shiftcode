...
const TeamCard = ({name, position, socialLinks, image}) => {
 ...
 return (
    <div className="flex flex-col items-center p-8 transition-all duration-300 transform border border-borderColor bg-coprimary
    rounded-xl hover:border-primary group hover:bg-primary hover:-translate-y-2 hover:shadow-[0_20px_50px_-15px_rgba(47,214,108,0.35)]" data-aos="fade-up" data-aos-delay="300">
      <img
        className="object-cover w-32 h-32 rounded-full ring-4 ring-borderColor group-hover:ring-background/30 transition-all duration-300"
        src={image}
        alt={`${name} picture`}
      />

      <h1 className="mt-4 text-2xl font-semibold text-text group-hover:text-background transition-colors duration-300">
        {name}
      </h1>

      <p className="mt-2 capitalize text-muted group-hover:text-background/80 transition-colors duration-300">
        {position}
      </p>
          ...
          Object.entries(socialLinks).map(([platform, url]) =>
            socialIcons[platform] ? (
              <a
                ...
                aria-label={platform}
                className="mx-2 text-muted group-hover:text-background transition-colors duration-300 cursor-pointer hover:scale-110"
              >
                {socialIcons[platform]}
              </a>
            ) : null
